...
The process is split in 3 parts, corresponding to Sessions 3-4, 5-6, and an optional last part: 

1. *Part 1*: We use some of the survery questions (e.g. in this case the first 29 "attitude" questions) to find **key customer descriptors** ("factors") using *dimensionality reduction* techniques described in the [Dimensionality Reduction](http://inseaddataanalytics.github.io/INSEADAnalytics/CourseSessions/Sessions23/FactorAnalysisReading.html) reading of Sessions 3-4.
...
Before starting, make sure you have pulled the [course files](https://github.com/InseadDataAnalytics/INSEADAnalytics) on your github repository.  As always, you can use the `help` command in Rstudio to find out about any R function (e.g. type `help(list.files)` to learn what the R function `list.files` does).


```{r setuplibraries, echo=FALSE, warning=FALSE}
# SET UP
...
suppressWarnings(source(paste(local_directory,"../../AnalyticsLibraries/heatmapOutput.R", sep = "/")))
...
ProjectData <- read.csv(paste0("../Sessions23/data/", datafile_name, ".csv")) # this contains only the matrix ProjectData
...
(All user inputs for this part should be selected in the code chunk in the raw .Rmd file here) 
...
dformat(round(ProjectDataFactor, 2)[1:min(max_data_report,nrow(ProjectDataFactor)), ])
```
<br>
The data we use here have the following descriptive statistics: 

```{r}
dformat(round(my_summary(ProjectDataFactor), 2))
```
...
**Answers:**
Answers to 1:
yes, we do see stronger correlations between the following responses:
6 and 7
3 and 9
13 and 14
16 and 17
...
To better visualise them, we will use what is called a "rotation". There are many rotations methods. In this case we selected the `r rotation_used` rotation. For our data, the `r factors_selected` selected factors look as follows after this rotation: 
...
Rotated_Factors_thres[abs(Rotated_Factors_thres) < MIN_VALUE]<-"-"
colnames(Rotated_Factors_thres)<- colnames(Rotated_Factors)
rownames(Rotated_Factors_thres)<- rownames(Rotated_Factors)

dformat(Rotated_Factors_thres)
```
...
dformat(NEW_ProjectData[1:min(max_data_report,nrow(NEW_ProjectData)), , drop=F])
```
...
```{r setupcluster, echo=FALSE, message=FALSE}
...
dformat(round(euclidean_pairwise))
```
...
do.call(grid.arrange, lapply(variables_to_plot, function(n) {
...
part_to_plot = 1:20
ggplot(df1[part_to_plot,], aes(x=index, y=distances)) + geom_line() + xlab("Number of Components") +ylab("Distances")
...
dformat(round(cluster.profile, 2))
```
<br>
...
dformat(round(cluster_profile_ratios-1, 2))
```
...
We will now use the [classification analysis ](http://inseaddataanalytics.github.io/INSEADAnalytics/Sessions67/ClassificationAnalysisReading.html) methods to understand the key purchase drivers for boats (a similar analysis can be done for recommendation drivers). For simplicity we do not follow the "generic" steps of classification discussed in that reading, and only consider the classification and purchase drivers analysis for the segments we found above. 
...
df<-data.frame(Percentile = 100*xaxis, Profit = yaxis)
c3(df, x="Percentile", y="Profit") %>% c3_line('spline')
...
dformat(round(tail(Log_Drivers,-1), 2))
